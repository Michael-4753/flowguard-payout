...
eyebrow(s, "目录"); title(s, "今天用 5 分钟讲清 11 件事")
toc = ["真实问题：谁在痛、痛在哪","产品介绍：FlowGuard 是什么","核心功能①：收款方信息预检-AI Agent",
       "核心功能②：多路径智能路由推荐引擎","核心功能③：分阶段里程碑付款工作台","核心功能④：统一结算状态与对账看板",
       "AI 覆盖一览：四痛点→四能力","现场 Demo：你能看到什么","技术架构：怎么实现的","合规边界：我们不碰什么（关键）","总结与可继续方向"]
...
bullets_grid(s, [
    ...
    ("② 莫名被退，发起时才知道","被退回的以转账（尤其打款给个人收款人）最常见；退回多是收款信息/合规校验问题，却发起后才发现，代价几天到一周 → AI: draft the follow-up"),
...
feature("核心功能② · AI 为何必要","多路径智能路由推荐引擎","统一池管理多类持牌结算通道，输出最优路径对比建议——平台不执行付款。",
        [("统一通道池","传统电汇、跨境人民币、第三方跨境支付通道；境外主体可选通道含境外持牌数字结算服务商方案。"),
...
feature("核心功能③ · 成果可演示","分阶段里程碑付款工作台","外包/项目付款节点台账管理，按里程碑进度设置触发条件；条件结算管理。",
        [("节点台账管理","外包/项目付款节点集中管理，进度与金额一目了然。"),
         ("里程碑触发条件","按里程碑进度设置付款触发条件：待开始→进行中→待校验→已校验。"),
         ("完成校验→放款提醒","校验通过后向财务推送放款提醒，一键深链跳转付款流程。"),
         ("合规边界","实际放款操作全部在银行/持牌结算机构完成——平台不托管、不放款。")],
