...
export interface HackSlide {
  ...
  /** demo slide: one-line description of what each app page does. */
...
export const HACK_DECK: HackSlide[] = [
  ...
  },
  {
    ...
    video: "/eazo-assets/att_4rnsqqdleomumbxor6e-feat-precheck-demo-v3.mp4",
  },
  {
    id: "f2",
    kind: "feature",
    eyebrow: "核心功能② · 事中破黑箱",
    title: "结算链路透明化追踪",
    subtitle: "逐跳看清资金流经的每一层中间行，破除“钱卡在哪看不见”的黑箱；并在多持牌通道间比价选路。",
    bullets: [
      { head: "资金流逐跳可视", body: "从起点到收款人，逐一层展开经手行、耗时与逐层扣费后余额，钱走到哪一目了然。" },
      { head: "黑箱卡点标注", body: "逐跳标出透明度（清晰/部分/黑箱）与滞留卡点，付款后还能定位钱现在卡在哪家中间行。" },
      { head: "多通道比价选路", body: "看清链路后在多类持牌结算通道间，按费用/时效/退回率自动比价，给出可解释的最优路径——平台不执行付款。" },
  ...
  },
  {
    ...
    subtitle: "一条真实付款主线，串起 App 的六个页面——每页一句话说清它负责什么。",
    pages: [
      { name: "首页 · 概览", body: "看板式首页：待办付款、进行中笔数与整体退回风险一屏概览，作为进入各流程的入口。" },
      { name: "付款向导 · 退回预检", body: "录入收款方后，规则引擎给出退回概率与命中因子，AI 补充语义风险，先拦截而非先失败。" },
      { name: "付款向导 · 比价选路", body: "在多类持牌通道间按费用/时效/退回率排序推荐路径，AI 用自然语言解释为什么选它。" },
      { name: "结算对账看板", body: "逐跳追踪资金流经的中间行、标注黑箱卡点，聚合进度与凭证，自动核销并导出对账单。" },
      { name: "双人放行审批", body: "大额付款进入双人/多级复核，指令生成前留痕，责任可追溯——平台不经手资金。" },
      { name: "AI 核实工单 / 案例", body: "把检查结论翻译成通俗说明与修复步骤，一键生成供应商核实工单，沉淀为可复用案例。" },
    ],
